...
from bertabs.modeling_bertabs import BertAbs, build_predictor
from transformers import BertTokenizer
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM


from bertabs.utils_summarization import (
    CNNDMDataset,
    build_mask,
...
def save_summaries(summaries, path, original_document_name):
    ...
    for summary, document_name in zip(summaries, original_document_name):
        print("original_document_name is " + str(document_name))
        # Prepare the summary file's name
        if "." in document_name:
            bare_document_name = ".".join(document_name.split(".")[:-1])
            bare_document_name = bare_document_name.replace("_raw", "")
            extension = document_name.split(".")[-1]
            name = bare_document_name + "_summary." + extension
            print("Summary file name will be: " + str(name))
        else:
            name = document_name + "_summary"

        # file_path = os.path.join(path, name)
        # print("file_path will be: " + str(file_path))
        # with open(file_path, "w") as output:
        with open(os.path.join(path, name), "w") as output:
...
def format_rouge_scores_old(scores):
    return """\n
****** ROUGE SCORES ******
